      ...
      <h2 className='text-center mb-2 text-lg'>For Which you want to create your personal study material</h2>
        <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5 mt-6'>
            {options.map((op,index)=>(
                <>
                <div key={index} className={`flex flex-col items-center justify-center border rounded-xl hover:border-primary hover:border-3 cursor-pointer ${op?.name === selected && 'border-primary' }`} onClick={()=>{setSelected(op.name) ; selectedStudyType(op.name)} }>
